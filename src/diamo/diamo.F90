...
module oxypom_diamo
   ...
   type, extends(type_base_model), public :: type_oxypom_diamo
...
contains
   ...
   subroutine initialize(self, configunit)
      class(type_oxypom_diamo), intent(inout), target :: self
      ...
      call self%register_state_variable(self%id_PHY, 'PHY', 'mmol C m-3', 'phytoplankton concentration', 1.80_rk, &
        minimum=0.0_rk, maximum=1000.0_rk, vertical_movement=self%sinkingPHY*d_per_s)
      call self%register_state_variable(self%id_DET, 'DET', 'mmol C m-3', 'detritus concentration', 1.80_rk, &
        minimum=0.0_rk, maximum=1000.0_rk, vertical_movement=self%sinkingDET*d_per_s)
      call self%register_state_variable(self%id_OXY, 'OXY', 'mmol O2 m-3', 'oxygen concentration', 400.0_rk, &
        minimum=0.0_rk, maximum=600.0_rk, vertical_movement=0.0*d_per_s)
      ...
      call self%add_to_aggregate_variable(standard_variables%attenuation_coefficient_of_photosynthetic_radiative_flux, &
        self%id_PHY, scale_factor=self%attenuationPHY)
      call self%add_to_aggregate_variable(standard_variables%attenuation_coefficient_of_photosynthetic_radiative_flux, &
        self%id_DET, scale_factor=self%attenuationDET)
      call self%add_to_aggregate_variable(standard_variables%attenuation_coefficient_of_photosynthetic_radiative_flux, &
        self%id_OXY, scale_factor=0.0_rk)
      call self%add_to_aggregate_variable(standard_variables%attenuation_coefficient_of_photosynthetic_radiative_flux, &
        self%attenuationSIM*self%SIM)
   ...
   subroutine do(self, _ARGUMENTS_DO_)
      class(type_oxypom_diamo), intent(in) :: self
